...
## Source: https://towardsdatascience.com/exploratory-data-analysis-guide-4f9367ab05e5
# ========== Packages ==========
import pandas as pd
...
def partition(df, target, test_size=.2, seed=8):
    ...
    ----------
    df:
        A pandas DataFrame to partition
    target:
        A string specifying the name of the target column
    test_size:
        (optional) An integer for number of test examples or a float
        for proportion of test examples
    seed:
        (optional) An integer to set seed for reproducibility
...
def plot_discrete(df, feature, target, orientation='v', figsize=(14, 4)):
    ...
    feature:
        A string specifying the name of the feature column
    target:
        A string specifying the name of the target column
    orientation:
        (optional) 'h' for horizontal and 'v' for  orientation of bars
    figsize:
        (optional) A tuple specifying the shape of the plot
